module Avalanche
  module Stats
    class AvalancheJobBasedStats < Avalanche::Stats::BasedStats
      ...
      def build_query(columns, date: @dates.last)
        ...
        columns.each do |column|
          ...
          case column
          when :error_message
            request = request.select("MAX(avalanche_jobs.error_message)")
          when :log
            request = request.select("MAX(avalanche_jobs.log)")
          when :job_id
      ...
      def request_filters(date)
        ...
        if @filters[:perform_at_before]
          request = request.perform_at_before(@filters[:perform_at_before])
        end

        if @filters[:perform_at_after]
          request = request.perform_at_after(@filters[:perform_at_after])
        end
      ...
      def segmentation_select(request)
        @segmentations.each do |segmentation|
          ...
          when :job_id
            request = request.select('avalanche_jobs.agent_id')
          when :action_name
      ...
      def segmentation_group(request)
        ...
        @segmentations.each do |segmentation|
          case segmentation
            ...
            when :agent_id
              groups << 'avalanche_jobs.agent_id'
            when :action_name
